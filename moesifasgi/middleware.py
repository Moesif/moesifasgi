...
class MoesifMiddleware(BaseHTTPMiddleware):
    ...
    def log_checkpoint(self, request_start_time, description: str):
        if self.DEBUG and request_start_time is not None:
            current_time = time.perf_counter()
            elapsed_time_ms = (current_time - request_start_time) * 1000
    ...
    async def get_body(self, request: Request) -> bytes:
        ...
        self.log_checkpoint(request.start_time, "Get Body")
        return body
    ...
    async def get_form_data(self, request: Request):
        try:
            ...
            self.log_checkpoint(request.start_time, "Get form data")
    ...
    def prepare_response_content(self, body, request_start_time):
        ...
        self.log_checkpoint(request_start_time, "Prepare response content")
    ...
    async def dispatch(self, request, call_next):
        ...
        self.log_checkpoint(request.start_time, "Request time", )

        # Request headers
        request_headers = dict(request.headers)
        self.log_checkpoint(request.start_time, "Request header")

        # Check if multipart/form-data payload
        is_multi_part_request_upload = self.parse_body.is_multi_part_upload(request_headers)
        self.log_checkpoint(request.start_time, "Multipart form-data Request")
        ...
        self.log_checkpoint(request.start_time, "Request body")
        ...
        self.log_checkpoint(request.start_time, "Request Model")
        ...
        if 'blocked_by' in governed_response:
            ...
            self.log_checkpoint(request.start_time, "Blocked By")
        else:
            # Call the next middleware
            response = await call_next(request)
            self.log_checkpoint(request.start_time, "Next mw")

        # response time
        response_time = self.get_time()
        if self.DEBUG:
            logger.info(f"event response time: {str(response_time)}")
        self.log_checkpoint(request.start_time, "Response time")
        ...
        if skip:
            ...
            return response
        self.log_checkpoint(request.start_time, "Skip event")

        # Response headers
        response_headers = dict(response.headers)
        self.log_checkpoint(request.start_time, "Response header")
        
        # Check if multipart/form-data payload
        is_multi_part_response_upload = self.parse_body.is_multi_part_upload(response_headers)
        self.log_checkpoint(request.start_time, "Multipart form-data Response")
        ...
        self.log_checkpoint(request.start_time, "Response Model")
        ...
        self.log_checkpoint(request.start_time, "Mask Event and sampling")
        ...
        self.log_checkpoint(request.start_time, "Response body and/or log event")
        
        self.log_checkpoint(request.start_time, "Dispatch")
